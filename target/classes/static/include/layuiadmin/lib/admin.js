...
layui.define("view", function(e) {
	var a = layui.jquery,
		...
		F = {
			...
			sendAuthCode: function(e) {
				...
				var i, t = e.seconds,
					l = a(e.elem),
					n = function(a) {
						t--, t < 0 ? (l.removeClass(f).html("获取验证码"), t = e.seconds, clearInterval(i)) : l.addClass(f).html(t + "秒后重获"),
			...
			sideFlexible: function(e) {
				var i = u,
					t = a("#" + m),
					n = F.screen();
				"spread" === e ? (t.removeClass(x).addClass(g), n < 2 ? i.addClass(v) : i.removeClass(v), i.removeClass(C)) : (t
					.removeClass(g).addClass(x), n < 2 ? i.removeClass(C) : i.addClass(C), i.removeClass(v)), layui.event.call(
					this, l.MOD_NAME, "side({*})", {
						status: e
					})
			},
			escape: function(e) {
			...
			theme: function(e) {
				var t = (l.theme, layui.data(l.tableName)),
					...
					r = i([".shortcut",
						"{background-color:{{d.color.shortcut}} !important;}",".layui-side-menu,", ".layadmin-pagetabs .layui-tab-title li:after,",
						".layadmin-pagetabs .layui-tab-title li.layui-this:after,", ".layui-layer-admin .layui-layer-title,",
						".layadmin-side-shrink .layui-side-menu .layui-nav>.layui-nav-item>.layui-nav-child",
						"{background-color:{{d.color.main}} !important;}", ".layui-nav-tree .layui-this a,",
						".layui-nav-tree .layui-nav-child a:hover,", ".layui-nav-tree .layui-nav-child dd.layui-this a", "{background-color:{{d.color.selected}} !important;}",
				...
				"styleSheet" in s ? (s.setAttribute("type", "text/css"), s.styleSheet.cssText = r) : s.innerHTML = r, s.id = n,
			...
			initTheme: function(e) {
				var a = l.theme;
				e = e || 0, a.color[e] && (a.color[e].index = e, F.theme({
					color: a.color[e]
				}))
			},
			tabsPage: {},
			...
			tabsBodyChange: function(e, a) {
				a = a || {}, F.tabsBody(e).addClass(d).siblings().removeClass(d), P.rollPage("auto", e), layui.event.call(this,
					l.MOD_NAME, "tabsPage({*})", {
						url: a.url,
						text: a.text
					})
		...
		P = F.events = {
			...
			serach: function(e) {
				e.off("keypress").on("keypress", function(a) {
					if (this.value.replace(/\s/g, "") && 13 === a.keyCode) {
						...
						i += this.value, t = t + ' <span style="color: #FF5722;">' + F.escape(this.value) + "</span>", layui.index.openTabsPage(
								i, t), P.serach.keys || (P.serach.keys = {}), P.serach.keys[F.tabsPage.index] = this.value, this.value ===
							P.serach.keys[F.tabsPage.index] && P.refresh(e), this.value = ""
					}
				})
			},
			message: function(e) {
			...
			fullscreen: function(e) {
				...
				if (t.hasClass(a)) {
					var l = document.body;
					l.webkitRequestFullScreen ? l.webkitRequestFullScreen() : l.mozRequestFullScreen ? l.mozRequestFullScreen() : l
						.requestFullScreen && l.requestFullscreen(), t.addClass(i).removeClass(a)
				} else {
					var l = document;
					l.webkitCancelFullScreen ? l.webkitCancelFullScreen() : l.mozCancelFullScreen ? l.mozCancelFullScreen() : l.cancelFullScreen ?
						l.cancelFullScreen() : l.exitFullscreen && l.exitFullscreen(), t.addClass(a).removeClass(i)
				}
			},
			about: function() {
				F.popupRight({
			...
			closeOtherTabs: function(e) {
				...
				"all" === e ? (a(z + ":gt(0)").remove(), a(h).find("." + b + ":gt(0)").remove(), a(z).eq(0).trigger("click")) :
	...
	! function() {
		...
		e.theme ? F.theme(e.theme) : l.theme && F.initTheme(l.theme.initColorIndex), "pageTabs" in layui.setter || (layui.setter
				.pageTabs = !0), l.pageTabs || (a("#LAY_app_tabs").addClass(c), u.addClass("layadmin-tabspage-none")), s.ie && s.ie <
			10 && n.error("IE" + s.ie + "下访问可能不佳，推荐使用：Chrome / Firefox / Edge 等高级浏览器", {
				offset: "auto",
				id: "LAY_errorIE"
			})
		...
		e.siblings(".layui-nav-child")[0] && u.hasClass(C) && (F.sideFlexible("spread"), layer.close(e.data("index"))), F
			.tabsPage.type = "nav"
